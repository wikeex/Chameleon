...
class ChiaMonitor(Monitor):
    def __init__(self, ca_file, cert_file, key_file, plots_quantity: int,
                 host: str = 'localhost', port: int = 8560, interval: int = 30):
        super().__init__(interval)
        self.url = f'https://{host}:{port}/get_plots'
        self.plots_quantity = plots_quantity
    ...
    async def monitor(self):
        while True:
            try:
                ...
                if current_plots_count < self.plots_quantity:
                    logger.error(
                        f'quantity of plots is abnormal, current: {current_plots_count}, except: {self.plots_quantity}'
                    )
                    await self._alert(f'plots数量异常，当前数量：{current_plots_count}，预期数量：{self.plots_quantity}')
                else:
